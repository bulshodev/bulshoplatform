    ...
    public function setStake()
    {
        ...
        if ($checkStakeLimits["status"] != "ok") {
            ...
            $data = array(
                ...
                'stake_total'       =>  sprintf('%s', number_format((float)$this->Ticket->getStake() + $this->App->calculatePercentageAfter($this->Ticket->getStake(), Configure::read('Settings.service_fee')), intval(Configure::read('Settings.balance_decimal_places')), '.', '')),
                'service_fee'       =>  sprintf('%s', number_format((float)$this->App->calculatePercentageAfter($this->request->query['stake'], Configure::read('Settings.service_fee')), intval(Configure::read('Settings.balance_decimal_places')), '.', '')),
                'possible_wining'   =>  sprintf('%s', number_format((float)$this->Ticket->getWinning($this->Ticket->getTicketType(), $this->Ticket->getOdds(), $this->Ticket->getStake()), intval(Configure::read('Settings.balance_decimal_places')), '.', ''))
            );
        }
