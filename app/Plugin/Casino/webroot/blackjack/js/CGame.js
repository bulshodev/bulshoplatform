function CGame(oData){
    ...
    this.attachCardToDeal = function(pStartingPoint,pEndingPoint,bDealer,iCardCount){
            ...
            if(bDealer){
                //DEALER CARDS
                var dealtCards = JSON.parse(localStorage.getItem('dealer_cards'));
                iCard = dealtCards[dealtCards.length - 1];
                ...
                //PLAYER CARDS
				var dealtCards = JSON.parse(localStorage.getItem('player_cards'));
                iCard = dealtCards[dealtCards.length - 1];
                oCard.setInfo(pStartingPoint,pEndingPoint,iCard,s_oGameSettings.getCardValue(iCard),bDealer,iCardCount);
    ...
    this._dealing = function(){
        if(_iCardIndexToDeal<_aCurActiveCardOffset.length*2){
                ...
                if((_iCardIndexToDeal%_aCurActiveCardOffset.length) === 1){
                    ...
                    var iCard;
					if (_iCardDealedToDealer === 1) {
						iCard = JSON.parse(localStorage.getItem('dealer_cards'))[0];
					} else {
						dealerSecondCard = [pStartingPoint, pEndingPoint, _iCardDealedToDealer];
						iCard = 1;
					}
					...
					if(_iCardIndexToDeal == 2) {
						var iCard = JSON.parse(localStorage.getItem('player_cards'))[1];
					} else {
						var iCard = JSON.parse(localStorage.getItem('player_cards'))[0];
					}
					
                    oCard.setInfo(pStartingPoint,_oSeat.getAttachCardOffset(),iCard,s_oGameSettings.getCardValue(iCard),false,_oSeat.newCardDealed());
    ...
    this.hitDealer = function(){
		request = $.ajax({
			type: "post",
			url: "/eng/casino/games/blackjackDealDealer",
			dataType: "json",
		});
		...
		request.done(function (response, textStatus, jqXHR){
			var dealtCards = JSON.parse(localStorage.getItem('dealer_cards'));
			dealtCards.push(response);
			localStorage.setItem('dealer_cards', JSON.stringify(dealtCards));
			var pStartingPoint=new CVector2(_oStartingCardOffset.getX(),_oStartingCardOffset.getY());
			var pEndingPoint=new CVector2(_oDealerCardOffset.getX()+((CARD_WIDTH+3)*_iCardDealedToDealer),_oDealerCardOffset.getY());
			_iCardDealedToDealer++;

			self.attachCardToDeal(pStartingPoint,pEndingPoint,true,_iCardDealedToDealer);

			self.changeState(STATE_GAME_HITTING);
    ...
    this._passTurnToDealer = function(){
		request = $.ajax({
			...
			request.done(function (response, textStatus, jqXHR){
				...
				var dealtCards = JSON.parse(localStorage.getItem('dealer_cards'));
				var newCard = dealtCards[dealtCards.length - 1];
				
				_aDealerCards[1].setInfo(dealerSecondCard[0],dealerSecondCard[1],newCard,s_oGameSettings.getCardValue(newCard),true,dealerSecondCard[2]);
				_iDealerValueCard -= 2;
				_iDealerValueCard += _aDealerCards[1].getValue();
				_oInterface.refreshDealerCardValue(_iDealerValueCard);
				if(_aDealerCards[1].getValue() === 11){
					_iAcesForDealer++;
				}
    ...
    this.onDeal = function(){
		...
		var data = {
			"amount": _oSeat.getCurBet()
		};
		request = $.ajax({
			type: "post",
			url: "/eng/casino/games/blackjackDeal",
			dataType: "json",
		...
		var self = this;
		request.done(function (response, textStatus, jqXHR){
			localStorage.setItem('player_cards', JSON.stringify([response[0], response[2]]));
			localStorage.setItem('dealer_cards', JSON.stringify([response[1]]));
    ...
    this.onHit = function(){
		request = $.ajax({
			type: "post",
			url: "/eng/casino/games/blackjackDealPlayer",
			dataType: "json",
		});
		request.fail(function (jqXHR, textStatus, errorThrown){
			alert('Server is not available right now. Please try again later!');
		...
		request.done(function (response, textStatus, jqXHR){
			var dealtCards = JSON.parse(localStorage.getItem('player_cards'));
			dealtCards.push(response);
			localStorage.setItem('player_cards', JSON.stringify(dealtCards));
			
			var pStartingPoint=new CVector2(_oStartingCardOffset.getX(),_oStartingCardOffset.getY());
			
			var pEndingPoint=new CVector2(_oSeat.getAttachCardOffset().getX(),_oSeat.getAttachCardOffset().getY());

			self.attachCardToDeal(pStartingPoint,pEndingPoint,false,_oSeat.newCardDealed());
    ...
    this.onStand= function(){
		if (_bSplitActive) {
			request = $.ajax({
				type: "post",
				url: "/eng/casino/games/blackjackSplitStand",
				dataType: "json",
			});
			request.fail(function (jqXHR, textStatus, errorThrown){
				alert('Server is not available right now. Please try again later!');
    ...
    this.onDouble = function(){
		request = $.ajax({
			type: "post",
			url: "/eng/casino/games/blackjackDouble",
			dataType: "json",
		});
		request.fail(function (jqXHR, textStatus, errorThrown){
			alert('Server is not available right now. Please try again later!');
    ...
    this.onSplit = function(){
		request = $.ajax({
			type: "post",
			url: "/eng/casino/games/blackjackSplit",
			dataType: "json",
		});
		request.fail(function (jqXHR, textStatus, errorThrown){
			alert('Server is not available right now. Please try again later!');
		...
		request.done(function (response, textStatus, jqXHR){
			...
			_oSeat.split();
			self.changeState(STATE_GAME_SPLIT);
		});	
    };
