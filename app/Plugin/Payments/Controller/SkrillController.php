...
//    public $components = array('Payments.InterSwitch');
    ...
    public function submitPayment()
    {
        ...
        if (!$this->request->is('post')) {
            $this->redirect(array('plugin' => null, 'controller' => 'deposits', 'action' => 'index'));
            die();
        }
        ...
        try {
            ...
            $transaction = $gateway->purchase([
                ...
                'status_url'        =>  $this->config['Config']['callback1'],
                'return_url'        =>  $this->config['Config']['return_url'],
                'cancel_url'        =>  $this->config['Config']['cancel_url'],
                'language'          =>  'EN',
                'amount'            =>  floatval($this->request->data[$this->name]['amount']),
                'currency'          =>  'USD',
                'recipient_description' => 'Deposit funds',
                'details'           =>  []
            ]);
