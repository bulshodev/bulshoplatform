                ...
                <ul class="bets-panel no-border-bottom">
                    <li>
                        ...
                        <div class="right pakelti">
                            ...
                            <?php if($Event["Event"]["markets_count"] > 0): ?>
                                <a href="<?php echo Router::url(array('language' => Configure::read('Config.language'), 'plugin' => 'events', 'controller' => 'events', 'action' => 'display', $Event["Event"]["id"])); ?>" class="plus click-add"><?=sprintf("+%d markets", $Event["Event"]["markets_count"])?></a>
                            <?php else: ?>
                                <a href="<?php echo Router::url(array('language' => Configure::read('Config.language'), 'plugin' => 'events', 'controller' => 'events', 'action' => 'display', $Event["Event"]["id"])); ?>" class="plus click-add"><?=sprintf("+")?></a>
                            <?php endif; ?>
                            <div class="clear"></div>
                        </div>
                        <div class="clear"></div>
