<li>
    ...
    <div class="right pakelti">
        ...
        <?php if(isset($Event["markets_count"]) && $Event["markets_count"] > 0): ?>
            <a href="<?php echo Router::url(array('language' => Configure::read('Config.language'), 'plugin' => $Event["type"] == 1 ? 'events' : null, 'controller' => $Event["type"] == 1 ? 'events' : 'live', 'action' => $Event["type"] == 1 ? 'display' : 'display_event', $Event["id"])); ?>" class="plus click-add"><?=sprintf("+%d markets", $Event["markets_count"])?></a>
        <?php else: ?>
            <a href="<?php echo Router::url(array('language' => Configure::read('Config.language'), 'plugin' => $Event["type"] == 1 ? 'events' : null, 'controller' => $Event["type"] == 1 ? 'events' : 'live', 'action' => $Event["type"] == 1 ? 'display' : 'display_event', $Event["id"])); ?>" class="plus click-add"><?=sprintf("+")?></a>
        <?php endif; ?>
