                                        ...
                                        <?php if (!empty($data)): ?>
                                            ...
                                            <table class="table table-bordered table-hover" cellpadding="0" cellspacing="0">
                                                ...
                                                foreach ($data as $row):
                                                    ...
                                                    <tr <?php if(strtotime($row['Event']['date']) < time()):?>style="background-color: #f5f5f5;"<?php endif;?>>
                                                        ...
                                                        <td class="<?php echo $class; ?>">
                                                            <input name="active[]" data-id="<?=$row["Event"]["id"];?>" type="checkbox" <?php if($row["Event"]["active"] == 1):?>checked="checked"<?php endif;?> type="checkbox">
                                                        </td>
                                            ...
                                            <script type="text/javascript">
                                                $(document).ready(function() {
                                                    var update = "<?php echo $this->Html->url(array('language' => Configure::read('Config.language'), 'plugin' => 'events', 'controller' => 'events', 'action' => 'edit'));?>";

                                                    $('input[name="active[]"]').change(function() {
                                                        $.post( update + '/' + $(this).attr('data-id'), { 'Event' : { id : $(this).attr('data-id'), active: $(this).is(':checked') ? 1 : 0 } } );
                                                    });
                                                });
                                            </script>
                                        <?php else: ?>
