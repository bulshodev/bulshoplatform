...
<!--                                                        <td class="--><?php //echo $class; ?><!--">--><?php //echo $countries[$row['Country']['id']]["Country"]["name"]; ?><!--</td>-->
                                                        ...
                                                        <td class="<?php echo $class; ?>">
                                                            <input name="active[]" data-id="<?=$row["League"]["id"];?>" type="checkbox" <?php if($row["League"]["active"] == 1):?>checked="checked"<?php endif;?> type="checkbox">
                                                        </td>
                                            ...
                                            <script type="text/javascript">
                                                $(document).ready(function() {
                                                    var update = "<?php echo $this->Html->url(array('language' => Configure::read('Config.language'), 'controller' => 'leagues', 'action' => 'edit'));?>";

                                                    $('input[name="active[]"]').change(function() {
                                                        $.post( update + '/' + $(this).attr('data-id'), { 'League' : { id : $(this).attr('data-id'), active: $(this).is(':checked') ? 1 : 0 } } );
                                                    });
                                                });
                                            </script>
                                        <?php else: ?>
